...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params = nn.ModuleDict({
            "lat3": nn.Conv2d(in_channels=64, out_channels=self.out_channels, kernel_size=1, stride=1),
            "lat4": nn.Conv2d(in_channels=160, out_channels=self.out_channels, kernel_size=1, stride=1),
            "lat5": nn.Conv2d(in_channels=400, out_channels=self.out_channels, kernel_size=1, stride=1),
            "out3": nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=3,
                              stride=1, padding="same"),
            "out4": nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=3,
                              stride=1, padding="same"),
            "out5": nn.Conv2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=3,
                              stride=1, padding="same")
    ...
    def forward(self, images: torch.Tensor):
        ...
        p5 = self.fpn_params['lat5'](backbone_feats['c5'])
        p4 = self.fpn_params['lat4'](backbone_feats['c4'])
        p4 += F.interpolate(p5, size=p4.shape[2:])
        p3 = self.fpn_params['lat3'](backbone_feats['c3'])
        p3 += F.interpolate(p4, size=p3.shape[2:])
        fpn_feats['p5'] = self.fpn_params['out5'](p5)
        fpn_feats['p4'] = self.fpn_params['out4'](p4)
        fpn_feats['p3'] = self.fpn_params['out3'](p3)
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        # Replace "pass" statement with your code
        h_arr = torch.arange(feat_shape[2], dtype=dtype, device=device).repeat_interleave(feat_shape[3])
        w_arr = torch.arange(feat_shape[3], dtype=dtype, device=device).repeat(feat_shape[2])
        location_coords[level_name] = (torch.stack((w_arr, h_arr), dim=1) + 0.5) * strides_per_fpn_level[level_name]
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        cur_chan = in_channels
        for new_chan in stem_channels:
            stem_cls.append(nn.ReLU())
            stem_cls.append(nn.Conv2d(cur_chan, new_chan,
                                      kernel_size=3, stride=1, padding='same'))
            nn.init.normal_(stem_cls[-1].weight, mean=0.0, std=0.01)
            nn.init.constant_(stem_cls[-1].bias, 0.0)

            stem_box.append(nn.ReLU())
            stem_box.append(nn.Conv2d(cur_chan, new_chan,
                                      kernel_size=3, stride=1, padding='same'))
            nn.init.normal_(stem_box[-1].weight, mean=0.0, std=0.01)
            nn.init.constant_(stem_box[-1].bias, 0.0)
        ...
        self.pred_cls = nn.Conv2d(
            stem_channels[-1], num_classes, kernel_size=3, stride=1, padding='same'
        )
        self.pred_box = nn.Conv2d(
            stem_channels[-1], 4, kernel_size=3, stride=1, padding='same'
        )
        self.pred_ctr = nn.Conv2d(
            stem_channels[-1], 1, kernel_size=3, stride=1, padding='same'
        )
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
            cls_s = self.stem_cls(value)
            class_logits[key] = self.pred_cls(cls_s).permute(0, 2, 3, 1).flatten(start_dim=1, end_dim=-2)

            box_s = self.stem_box(value)
            boxreg_deltas[key] = self.pred_box(box_s).permute(0, 2, 3, 1).flatten(start_dim=1, end_dim=-2)
            centerness_logits[key] = self.pred_ctr(box_s).permute(0, 2, 3, 1).flatten(start_dim=1, end_dim=-2)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.zeros_like(gt_boxes[:, :4]).to(gt_boxes.device)
    deltas[:, 0] = (locations[:, 0] - gt_boxes[:, 0]) / stride
    deltas[:, 1] = (locations[:, 1] - gt_boxes[:, 1]) / stride
    deltas[:, 2] = -(locations[:, 0] - gt_boxes[:, 2]) / stride
    deltas[:, 3] = -(locations[:, 1] - gt_boxes[:, 3]) / stride
    deltas[(gt_boxes == -1).all(axis=1)] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness[(deltas == -1).all(axis=1)] = -1
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    cnts = box_iou(boxes, boxes)
    order = torch.argsort(scores, descending=True)
    mask = torch.zeros(size=scores.shape)
    keep = []
    for el in order:
        if mask[el] == 1:
            continue
        keep.append(el)
        mask[cnts[el] > iou_threshold] = 1

    keep = torch.LongTensor(keep).to(boxes.device)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return keep
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        img = {key: feat.shape for key, feat in self.backbone(images).items()}
        locations_per_fpn_level = get_fpn_location_coords(
            img, self.backbone.fpn_strides, device=images.device
        )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        matched_gt_boxes = [
            fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.fpn_strides, gt_box)
        ...
        for box in matched_gt_boxes:
            matched_gt_deltas.append({
                fpn_key: fcos_get_deltas_from_locations(
                    locations_per_fpn_level[fpn_key],
                    box[fpn_key],
                    self.backbone.fpn_strides[fpn_key], )
                for fpn_key in ['p3', 'p4', 'p5']})
        ...
        onehot = F.one_hot((matched_gt_boxes[:, :, 4] + 1).to(torch.int64), num_classes=self.num_classes + 1)[:, :, 1:]
        loss_cls = sigmoid_focal_loss(
            pred_cls_logits,
            targets=onehot.float(),
        )
        ...
        targ_cent = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4))
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits.view(-1), targ_cent, reduction="none")
        loss_ctr[targ_cent < 0] *= 0.0
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            best = level_pred_scores.max(axis=1)
            level_pred_scores = best.values
            level_pred_classes = best.indices
            ...
            level_pred_boxes[:, 0][level_pred_boxes[:, 0] > h] = h
            level_pred_boxes[:, 0][level_pred_boxes[:, 0] < h] = 0
            level_pred_boxes[:, 1][level_pred_boxes[:, 1] > w] = w
            level_pred_boxes[:, 1][level_pred_boxes[:, 1] < w] = 0
            level_pred_boxes[:, 2][level_pred_boxes[:, 2] > h] = h
            level_pred_boxes[:, 2][level_pred_boxes[:, 2] < h] = 0
            level_pred_boxes[:, 3][level_pred_boxes[:, 3] > w] = w
            level_pred_boxes[:, 3][level_pred_boxes[:, 3] < w] = 0
